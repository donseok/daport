...
describe("validateEditPatch", () => {
  ...
  it("drops an op that cannot be applied and keeps going, returning only the surviving op", () => {
    ...
    expect(res.patch).toHaveLength(1);   // C1: 버려진 op은 응답 patch에도 남으면 안 된다
    expect(res.patch[0]).toMatchObject({ path: "/elements/0/value" });
  });
  it("drops an out-of-bounds add and returns only the surviving op", () => {
    const res = validateEditPatch(report, raw([
      op("add", "/elements/5", { id: "t2", type: "text", x: 0, y: 0, w: 10, h: 5, value: "b" }),
      op("replace", "/elements/0/value", "ok"),
    ]));
    expect(res.patch).toHaveLength(1);
    expect(res.patch[0]).toMatchObject({ path: "/elements/0/value" });
    expect(res.next.elements).toHaveLength(1);
    expect(res.warnings.some((w) => w.includes("/elements/5"))).toBe(true);
  });
  it("renames a new element whose id collides, in the value and in later ops, and warns about it", () => {
    const res = validateEditPatch(report, raw([
    ...
    expect(res.warnings.some((w) => w.includes("t1") && w.includes(added.id))).toBe(true);
  });
  it("drops an op whose value fails JSON.parse, with a warning, and keeps the rest", () => {
    const res = validateEditPatch(report, raw([
      { op: "replace", path: "/elements/0/value", value: "{not json" },
      op("replace", "/page/width", 297),
    ]));
    expect(res.patch).toHaveLength(1);
    expect(res.next.page.width).toBe(297);
    expect(res.warnings.some((w) => w.includes("/elements/0/value"))).toBe(true);
  });
...
describe("validateGenerated", () => {
  ...
  it("resolves a ref nested inside a generated group", async () => {
    const res = await validateGenerated(empty, gen([
      { id: "g1", type: "group", x: 0, y: 0, w: 190, h: 20, children: [
        { id: "hd", type: "ref", x: 0, y: 0, w: 190, h: 20, ref: "header", version: 1, props: {} },
      ] },
    ]), lookup);
    const group = res.elements[0] as { children: { id: string; ref: string; version: number }[] };
    expect(group.children[0]).toMatchObject({ ref: "header", version: 3 });
    expect(res.components["header@3"]).toBeTruthy();
  });
