...
import { maskSqlNoise, hasUnterminatedNoise } from "./sql-lexer";
...
export function guardSql(sql: string): void {
  ...
  // "첫 키워드"는 스펙 4.2 기준 — `(SELECT ...)`처럼 괄호로 감싼 SELECT도 합법적인 읽기 전용 Oracle 구문이라
  // 첫 글자가 아니라 앞의 괄호·공백을 건너뛴 다음 첫 키워드를 본다
  ...
  if (hasUnterminatedNoise(sql)) throw deny("닫히지 않은 주석 또는 문자열이 있습니다");
}
