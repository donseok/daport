...
const connector: ManagedConnector = env.AGENT_FAKE === "1"
  ...
  : createDirectConnector({ name: "agent", via: "direct", host: need("ORACLE_HOST"), port: int("ORACLE_PORT", 1521), service: need("ORACLE_SERVICE"), user: need("ORACLE_USER"), secretRef: "ORACLE_PASSWORD" }, need("ORACLE_PASSWORD"));
...
const server = createAgentServer({ connector, token, version: AGENT_VERSION, maxConcurrency: int("AGENT_MAX_CONCURRENCY", 8), tls });
const port = int("AGENT_PORT", 8433);
...
const onSignal = () => shutdown(server, connector, process.exit);
